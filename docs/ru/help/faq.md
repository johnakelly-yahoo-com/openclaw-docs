...
Да, с помощью **маршрута нескольких агентов**. Bind each sender's WhatsApp **DM** (peer `kind: "direct"`, sender E.164 like `+15551234567`) to a different `agentId`, so each person gets their own workspace and session store. Ответы все еще поступают от **той же учетной записи WhatsApp**, и контроля доступа к DM (`channels.whatsapp.dmPolicy` / `channels.whatsapp.allowFrom`) является глобальным в каждой учетной записи WhatsApp. См. [Мульти-Агент маршрутизация](/concepts/multi-agent) и [WhatsApp](/channels/whatsapp).
