...
- **1時間後に有効期限が切れます** **1 時間後に失効** します。ボットは、新しいリクエストが作成されたときにのみペアリングメッセージを送信します（送信者あたり概ね 1 時間に 1 回）。
...
`device-pair` プラグインを使用している場合、初回のデバイスペアリングを Telegram だけで完結できます。

1. Telegram でボットにメッセージを送信します: `/pair`
2. ボットは 2 つのメッセージで返信します。1 つは手順の説明、もう 1 つは別送の **セットアップコード** メッセージ（Telegram で簡単にコピー／ペーストできます）。
3. スマートフォンで OpenClaw iOS アプリを開き → 設定 → Gateway を選択します。
4. セットアップコードを貼り付けて接続します。
5. Telegram に戻って: `/pair approve`

セットアップコードは base64 エンコードされた JSON ペイロードで、以下を含みます:

- `url`: Gateway の WebSocket URL（`ws://...` または `wss://...`）
- `token`: 短期間有効なペアリングトークン

有効期間中は、セットアップコードをパスワードと同様に扱ってください。
