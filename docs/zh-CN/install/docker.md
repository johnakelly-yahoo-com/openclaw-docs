...
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. 31. 参见 [沙箱](/gateway/sandboxing)。
...
container. 34. 网关驻留在你的主机上，但工具执行是隔离的：

- scope: `"agent"` by default (one container + workspace per agent)
- 35. scope：`"session"` 用于按会话隔离
...
36. 记住：如果你对工具使用了允许列表，请添加 `browser`（并从
    拒绝列表中移除），否则该工具仍会被阻止。
