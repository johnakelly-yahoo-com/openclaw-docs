---
summary: "43. OpenClaw 中的 OAuth：令牌交换、存储以及多账户模式"
read_when:
  - 44. 你想端到端地理解 OpenClaw OAuth
  - 45. 你遇到了令牌失效 / 登出问题
  - 46. 你需要 setup-token 或 OAuth 鉴权流程
  - 47. 你需要多个账户或按配置文件路由
title: "48. OAuth"
---
...
50. OpenClaw 支持通过 OAuth 实现“订阅式鉴权”（subscription auth），适用于提供该能力的服务商（尤其是 **OpenAI Codex（ChatGPT OAuth）**）。 For Anthropic subscriptions, use the **setup-token** flow. 9. 本页面说明：
...
```bash
12. openclaw models auth login --provider <id>
```
...
- 14. 认证配置（OAuth + API 密钥）：`~/.openclaw/agents/<agentId>/agent/auth-profiles.json`
