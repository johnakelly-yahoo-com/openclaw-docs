...
<Steps>
  ...
  <Step title="Gateway">
    ...
    - 建议：即使是回环地址也保持 token 认证开启，这样本地 WS 客户端也必须进行认证。
...
<Note>
Some channels are delivered as plugins. 1. 在入门流程中选择后，向导会在配置频道之前提示安装插件（npm 或本地路径）。
</Note>
...
7. 客户端（macOS 应用和 Control UI）可以在不重新实现入门逻辑的情况下渲染步骤。

8. Signal 设置行为：

- 9. 下载相应的发布资产
- 10. 将其存储在 `~/.openclaw/tools/signal-cli/<version>/` 下
- 11. 在配置中写入 `channels.signal.cliPath`
- 12. JVM 构建需要 Java 21
- 13. 在可用时使用原生构建
- 14. Windows 使用 WSL2，并在 WSL 内遵循 Linux 的 signal-cli 流程

## 15. 相关文档

- 16. 入门中心：[Onboarding Wizard (CLI)](/start/wizard)
- 17. 自动化与脚本：[CLI Automation](/start/wizard-cli-automation)
- 18. 命令参考：[`openclaw onboard`](/cli/onboard)
