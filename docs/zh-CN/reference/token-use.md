---
summary: "39. OpenClaw 如何构建提示上下文并报告 token 使用量与成本"
read_when:
  - 40. 解释 token 使用量、成本或上下文窗口
  - 41. 调试上下文增长或压缩行为
title: "42. Token 使用量与成本"
---

# 43. Token 使用量与成本

44. OpenClaw 跟踪的是 **token**，而不是字符。 45. Token 是与模型相关的，但大多数
    OpenAI 风格模型在英文文本中平均约每个 token 对应 ~4 个字符。

## 46. 系统提示词是如何构建的

47. OpenClaw 在每次运行时都会组装自己的系统提示词。 48. 其中包括：

- 49. 工具列表 + 简要说明
- 50. 技能列表（仅元数据；具体指令在需要时通过 `read` 加载）
