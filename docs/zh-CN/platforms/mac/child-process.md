...
macOS 应用默认**通过 launchd 管理 Gateway**，不会将 Gateway 作为子进程启动。 It first tries to attach to an already‑running
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` 在使用 `--profile`/`OPENCLAW_PROFILE` 时；支持旧版 `com.openclaw.*`）。
