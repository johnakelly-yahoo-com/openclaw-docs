...
- `canvasHost.port=19005` (hosila: `gateway.port+4`)
- `agents.defaults.workspace` standarti `--dev` ostida `setup`/`onboard` ishga tushirilganda `~/.openclaw/workspace-dev` ga aylanadi.
...
- Brauzer profili CDP portlari `browser.controlPort + 9 ..` dan avtomatik ajratiladi + 108\` (persisted per profile).
...
- WebChat — Gateway WebSocket bilan to‘g‘ridan-to‘g‘ri gaplashadigan, tarix, yuborish, bekor qilish va hodisalar uchun mo‘ljallangan mahalliy SwiftUI UI.
...
Windows o‘rnatmalari **WSL2** dan foydalanishi va yuqoridagi Linux systemd bo‘limiga amal qilishi kerak.
